...
!
MODULE ADVECTION_ADM_MOD

  use advection_mod, only: gridind_type
  use tapenade_iter, only: poprealarray, pushrealarray
...
!Advect once around the domain (if u = 1)
    DO t=1,nt
      DO j=1,nx
        CALL PUSHREALARRAY(xhalf(grid%fx(j)))
...
&         grid%fx(j))/dx
      END DO
      DO j=nx,1,-1
        CALL POPREALARRAY(xhalf(grid%fx(j)))
